...
static int	ctrl_c(char *filename, char *limiter, t_fd save_fd, int fd)
{
	int	exit_code;

	exit_code = handle_heredoc_error(filename, limiter, save_fd, fd);
	reset_fd(save_fd.save_stdin, save_fd.save_stdout);
	ft_clear(&limiter);
	return (exit_code);
}
