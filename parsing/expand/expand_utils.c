...
char	*process_segment(char *str, int *i, int start, char **envp, char *result)
{
	char	*prefsuf;
	char	*var;
...
char	*process_string(char *str, int *i, int *start, char **envp, char *result)
{
	while (str[*i] != '\0')
	{
		if (str[*i] == '\\' && str[*i + 1] == '$')
		...
		{
			result = process_segment(str, i, *start, envp, result);
			*start = *i;
		}
		else
			(*i)++;
...
char	*expand_var(char *str, char **envp)
{
	...
	if (!result)
		return (NULL);
	result = process_string(str, &i, &start, envp, result);
	if (start < i)
